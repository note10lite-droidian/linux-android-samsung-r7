				...
				CC_NOTI_TYPEDEF *usb_typec_info)
{
	...
	case CCIC_NOTIFY_ID_DP_CONNECT:
		/*
		 * This function is the path actually taken when
		 * CONFIG_USE_DISPLAYPORT_CCIC_EVENT_QUEUE is disabled, and it
		 * never assigned ccic_cable_state. The assignment only existed
		 * in the queued sibling, usb_typec_displayport_notification()
		 * below, which is dead code in this configuration.
		 *
		 * Consequence: even when a DP_CONNECT notification arrived,
		 * ccic_cable_state stayed at its default DETACH(0), so HPD
		 * handling always tripped over the "ccic cable is detached"
		 * check that follows the initial 10-11 ms usleep and bailed
